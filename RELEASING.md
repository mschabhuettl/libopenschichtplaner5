...
3. **Release ausführen:**
   ...
   Der Workflow committet Version + CHANGELOG auf `main`, pusht das annotierte
...
5. **Downstream-Pins:** `openschichtplaner5-api` und `openschichtplaner5`
   ...
   nach einem Release manuell anstoßen — `expected_lib` wartet die
   PyPI-CDN-Propagation ab:

   ```bash
   gh workflow run update-pins.yml -R mschabhuettl/openschichtplaner5-api -f expected_lib=X.Y.Z
   gh workflow run update-pins.yml -R mschabhuettl/openschichtplaner5 -f expected_lib=X.Y.Z
   ```
