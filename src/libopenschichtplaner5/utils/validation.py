...
class ValidationError:
    ...
    def __str__(self):
        return f"[{self.table***REMOVED***:{self.record_id***REMOVED***] {self.field***REMOVED***: {self.message***REMOVED***"
...
class ValidationReport:
    ...
    def add_error(self, error: ValidationError):
        ...
        error_key = f"{error.table***REMOVED***:{error.error_type***REMOVED***"
        self.statistics[error_key] = self.statistics.get(error_key, 0) + 1
    
    def add_warning(self, warning: ValidationError):
        """Add a warning to the report."""
        self.warnings.append(warning)
        warning_key = f"{warning.table***REMOVED***:{warning.error_type***REMOVED***:warning"
    ...
    def summary(self) -> str:
        ...
        lines = [
            ...
            f"  Total Errors: {len(self.errors)***REMOVED***",
            f"  Total Warnings: {len(self.warnings)***REMOVED***",
            "",
            "Error Statistics:"
        ]
        
        for key, count in sorted(self.statistics.items()):
            lines.append(f"  {key***REMOVED***: {count***REMOVED***")
...
class DataValidator:
    ...
    def __init__(self):
        ...
        self.field_rules = {
            "5EMPL": {
                "id": {"required": True, "type": int, "unique": True***REMOVED***,
                "name": {"required": True, "type": str, "min_length": 1***REMOVED***,
                "email": {"type": str, "pattern": r'^[\w\.-]+@[\w\.-]+\.\w+$', "required": False***REMOVED***,
                "empstart": {"type": date, "required": False***REMOVED***,
                "empend": {"type": date, "required": False, "after": "empstart"***REMOVED***,
            ***REMOVED***,
            "5ABSEN": {
                "id": {"required": True, "type": int, "unique": True***REMOVED***,
                "employee_id": {"required": True, "type": int, "foreign_key": "5EMPL.id"***REMOVED***,
                "date": {"required": True, "type": date***REMOVED***,
                "leave_type_id": {"required": True, "type": int, "foreign_key": "5LEAVT.id"***REMOVED***,
            ***REMOVED***,
            "5SPSHI": {
                "id": {"required": True, "type": int, "unique": True***REMOVED***,
                "employee_id": {"required": True, "type": int, "foreign_key": "5EMPL.id"***REMOVED***,
                "shift_id": {"required": True, "type": int, "foreign_key": "5SHIFT.id"***REMOVED***,
                "workplace_id": {"required": True, "type": int, "foreign_key": "5WOPL.id"***REMOVED***,
                "date": {"required": True, "type": date***REMOVED***,
            ***REMOVED***,
            "5NOTE": {
                "id": {"required": True, "type": int, "unique": True***REMOVED***,
            ...
            ***REMOVED***,
            "5GRASG": {
                "id": {"required": True, "type": int, "unique": True***REMOVED***,
                "employee_id": {"required": True, "type": int, "foreign_key": "5EMPL.id"***REMOVED***,
                "group_id": {"required": True, "type": int, "foreign_key": "5GROUP.id"***REMOVED***,
            ***REMOVED***,
            "5CYASS": {
                "id": {"required": True, "type": int, "unique": True***REMOVED***,
                "employee_id": {"required": True, "type": int, "foreign_key": "5EMPL.id"***REMOVED***,
                "cycle_id": {"required": True, "type": int, "foreign_key": "5CYCLE.id"***REMOVED***,
                "start": {"required": True, "type": date***REMOVED***,
                "end": {"required": True, "type": date, "after": "start"***REMOVED***,
            ***REMOVED***
        ***REMOVED***
    ...
    def _validate_table(self, table_name: str, records: List[Any]):
        ...
        rules = self.field_rules.get(table_name, {***REMOVED***)
        seen_ids = set()
            ...
            for field_name, field_rules in rules.items():
                ...
                if value is not None:
                    ...
                    if expected_type and not self._check_type(value, expected_type):
                        self.report.add_error(ValidationError(
                            table_name, record_id, field_name,
                            "invalid_type", f"Expected {expected_type.__name__***REMOVED***, got {type(value).__name__***REMOVED***",
                            value
                        ))
                    ...
                    if expected_type == str:
                        ...
                        if len(value.strip()) < min_length:
                            self.report.add_error(ValidationError(
                                table_name, record_id, field_name,
                                "too_short", f"Value too short (min: {min_length***REMOVED***)",
                                value
                            ))
                    ...
                    if field_rules.get("unique", False) and field_name == "id":
                        if value in seen_ids:
                            self.report.add_error(ValidationError(
                                table_name, record_id, field_name,
                                "duplicate_id", f"Duplicate ID found: {value***REMOVED***",
                                value
                            ))
                        seen_ids.add(value)
                ...
                for record in loaded_tables[table_name]:
                    ...
                    if value is not None and value not in id_indexes[ref_table]:
                        self.report.add_error(ValidationError(
                            ...
                            "invalid_reference", 
                            f"References non-existent {ref_table***REMOVED***.{ref_field***REMOVED***={value***REMOVED***",
                            value
                        ))
        ...
        for (emp_id, date), shift_list in employee_dates.items():
            if len(shift_list) > 1:
                self.report.add_warning(ValidationError(
                    ...
                    "multiple_shifts",
                    f"Employee {emp_id***REMOVED*** has {len(shift_list)***REMOVED*** shifts on {date***REMOVED***",
                    date
                ))
        ...
        for (emp_id, date), absence_list in employee_dates.items():
            if len(absence_list) > 1:
                self.report.add_error(ValidationError(
                    ...
                    "multiple_absences",
                    f"Employee {emp_id***REMOVED*** has {len(absence_list)***REMOVED*** absences on {date***REMOVED***",
                    date
                ))
            ...
            for i in range(len(cycles) - 1):
                ...
                if current.end and next_cycle.start and current.end >= next_cycle.start:
                    self.report.add_error(ValidationError(
                        ...
                        "overlapping_cycles",
                        f"Cycle overlaps with next cycle (ends {current.end***REMOVED***, next starts {next_cycle.start***REMOVED***)",
                        current.end
                    ))
...
class DataCleaner:
    ...
    def clean_record(cls, record: Any, table_name: str) -> Any:
        ...
        ***REMOVED***
        ...
        rules = cleaning_rules.get(table_name, {***REMOVED***)
