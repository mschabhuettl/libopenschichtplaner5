    ...
    for table_name in TABLE_NAMES:
        ...
        if dbf_path.exists():
            try:
                ...
                print(f"✅ Loaded {table_name} from {dbf_path}")
...
def perform_cross_referencing(dbf_dir: Path, employee_id: int = None) -> Dict[str, List[Any]]:
    ...
    for table_name, fields in lookup.items():
        for field, reference in fields.items():
            ...
            matched_data[f"{table_name}_{field}_to_{reference_table}"] = matched_pairs
        ...
        if matched_pairs:
            ...
            for pair in matched_pairs:
                if isinstance(pair[0], Note):
                    print(f" - Employee: {pair[0].employee_name} | Note ID: {pair[0].id} | {pair[0].text1}")
                else:
