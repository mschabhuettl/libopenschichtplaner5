...
class DataExporter:
    ...
    def export(self, data: Union[List[Dict], Dict], 
        ...
        if format == ExportFormat.CSV:
            ...
            raise ValueError(f"Unsupported format: {format***REMOVED***")
        ...
        for row_idx, record in enumerate(data, 2 if include_headers else 1):
            ...
            for col_idx, field in enumerate(fields, 1):
                ...
                if isinstance(value, (date, datetime)):
                    value = value.strftime(self.default_date_format if isinstance(value, date) else self.default_datetime_format)
    ...
    def to_html(self, data: Union[List[Dict], Dict],
        ...
        html_parts.append(f'<title>{html.escape(title)***REMOVED***</title>')
        ...
        html_parts.append(f'<h1>{html.escape(title)***REMOVED***</h1>')
        
        # Build table
        html_parts.append(f'<table class="{table_class***REMOVED***">')
        
        # Headers
        html_parts.append('<thead>')
        html_parts.append('<tr>')
        for field in fields:
            html_parts.append(f'<th>{html.escape(field)***REMOVED***</th>')
        html_parts.append('</tr>')
        html_parts.append('</thead>')
        ...
        for record in data:
            ...
            for field in fields:
                ...
                if isinstance(value, (date, datetime)):
                    value = value.strftime(self.default_date_format if isinstance(value, date) else self.default_datetime_format)
                html_parts.append(f'<td>{html.escape(str(value))***REMOVED***</td>')
            html_parts.append('</tr>')
        html_parts.append('</tbody>')
    ...
    def _flatten_dict(self, d: Dict[str, Any], parent_key: str = '', sep: str = '_') -> Dict[str, Any]:
        ...
        for k, v in d.items():
            ...
            new_key = f"{parent_key***REMOVED***{sep***REMOVED***{k***REMOVED***" if parent_key else k
            ...
            if isinstance(v, dict):
                ...
                if v and isinstance(v[0], str):
                    ...
                    items.append((f"{new_key***REMOVED***_count", len(v)))
            else:
...
class ReportExporter:
    ...
    def _export_schedule_excel(self, schedule_data: List[Dict[str, Any]], 
        ...
        title_cell = ws['A1']
        title_cell.value = f"Schedule for {employee_name***REMOVED***"
        ...
        for entry in schedule_data:
            ...
            if entry_date:
                ...
                if week_num != current_week:
                    ...
                    ws.merge_cells(f'A{row_idx***REMOVED***:E{row_idx***REMOVED***')
                    week_cell = ws.cell(row=row_idx, column=1)
                    week_cell.value = f"Week {week_num***REMOVED***"
                    week_cell.fill = PatternFill(start_color="CCCCCC", end_color="CCCCCC", fill_type="solid")
                    week_cell.font = Font(bold=True)
                    row_idx += 1
            
            # Add schedule entry
            shift_info = entry.get('5SHIFT_related', {***REMOVED***)
            workplace_info = entry.get('5WOPL_related', {***REMOVED***)
    ...
    def _export_summary_html(self, report_data: Dict[str, Any]) -> str:
        ...
        html_parts.append('<html>')
        html_parts.append('<head>')
        html_parts.append(f'<title>{report_data.get("title", "Report")***REMOVED***</title>')
        html_parts.append('<meta charset="utf-8">')
        html_parts.append('<style>')
        ...
        # Report content
        html_parts.append(f'<h1>{html.escape(report_data.get("title", "Report"))***REMOVED***</h1>')
        
        if "generated_at" in report_data:
            html_parts.append(f'<p class="metadata">Generated: {report_data["generated_at"]***REMOVED***</p>')
        ...
        for key, value in report_data.items():
            ...
            html_parts.append(f'<h2>{html.escape(key.replace("_", " ").title())***REMOVED***</h2>')
            ...
            if isinstance(value, dict):
                ...
                if value and isinstance(value[0], dict):
                    ...
                    for item in value:
                        html_parts.append(f'<li>{html.escape(str(item))***REMOVED***</li>')
                    html_parts.append('</ul>')
                html_parts.append('</div>')
            else:
                html_parts.append(f'<p>{html.escape(str(value))***REMOVED***</p>')
    ...
    def _dict_to_html_table(self, data: Dict[str, Any]) -> str:
        ...
        for key, value in data.items():
            html += '<tr>'
            html += f'<td class="key">{html.escape(key.replace("_", " ").title())***REMOVED***</td>'
            html += f'<td class="value">{html.escape(str(value))***REMOVED***</td>'
            html += '</tr>'
        html += '</table>'
        return html
    ...
    def _list_to_html_table(self, data: List[Dict[str, Any]]) -> str:
        ...
        html += '<thead><tr>'
        for field in fields:
            html += f'<th>{html.escape(field.replace("_", " ").title())***REMOVED***</th>'
        html += '</tr></thead>'
        ...
        for record in data:
            ...
            for field in fields:
                ...
                html += f'<td>{html.escape(str(value))***REMOVED***</td>'
            html += '</tr>'
        html += '</tbody>'
    ...
    def _export_summary_markdown(self, report_data: Dict[str, Any]) -> str:
        ...
        # Title
        lines.append(f"# {report_data.get('title', 'Report')***REMOVED***")
        lines.append("")
        
        if "generated_at" in report_data:
            lines.append(f"*Generated: {report_data['generated_at']***REMOVED****")
            lines.append("")
        ...
        for key, value in report_data.items():
            ...
            lines.append(f"## {key.replace('_', ' ').title()***REMOVED***")
            lines.append("")
            
            if isinstance(value, dict):
                for k, v in value.items():
                    lines.append(f"- **{k.replace('_', ' ').title()***REMOVED*****: {v***REMOVED***")
                lines.append("")
            elif isinstance(value, list):
                if value and isinstance(value[0], dict):
                    # Table
